...
for sd in SEEDS:
    ...
    for name in PROBES:
        ...
        res[name].append(mse / ref)
        pars[name].append(o["par"])
    geo = np.exp(np.mean([np.log(res[p][-1]) for p in PROBES]))
    print(f"seed {sd}: geo {geo:.3f}  worst {max(res[p][-1] for p in PROBES):.3f}",
          flush=True)
