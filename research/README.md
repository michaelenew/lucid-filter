...
| [`bias-channels/`](bias-channels/SUMMARY.md) | Delivered. Every noise channel here is second-moment; this is the first-moment one. A constant offset is identifiable only up to what a free response of the dynamics could already explain, so the channel is activated on that quotient — the drift, and *not* a single sensor's bias, which is gauge. Carried in two stages rather than as extra state, which is exact and costs nothing per bank member. Ships `LucidFilter(offsets=)`; the sensor-bias half is estimable and measurably not usable. |
