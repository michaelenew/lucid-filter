"""A/B: the shipped filter before the attribution grid (git HEAD source) against the ported filter.
    python probe_ab.py x {scalar|async|arm} [seed] [swap]"""
...
A = load_pre()
import lucid.filter.lucid as C
print("A = pre-port HEAD source, C = ported filter", flush=True)
...
if RIG == "scalar":
    ...
    for nm, mod in (("A", A), ("C", C)):
        ...
        for t in range(m54.T):
            st = f.update(Y[t], U[t]); ests.append(st.mean.copy()); xis.append(st.process_scale.copy()); pats.append(getattr(st, "patience", np.nan))
        dt = time.perf_counter() - t0; est = np.array(ests); pats = np.array(pats, float)
        class _R: pass
        ...
        if nm == "C" and np.all(np.isfinite(pats)):
            print("    patience per phase: " + "  ".join(f"{pnm} {pats[a:b].mean():.2f}" for pnm, a, b in m54.PHASES), "| switch at end %.3g" % getattr(st, "switch", np.nan), flush=True)
