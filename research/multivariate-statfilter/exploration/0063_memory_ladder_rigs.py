"""The MEMORY LADDER on every rig, against the shipped fixed forget.
    python probe_memrig.py {arm|scalar|async|dyn} [swap]   (env MEMS=... to set the rungs)"""
...
if RIG == "scalar":
    ...
    for seed in range(11, 23):
        ...
        for fg in FG:
            m = np.asarray(LF(fg).filter(y.reshape(-1, 1)).mean).reshape(-1)
            for k, sl in (("jump", slice(JA, JA + 40)), ("steady", slice(80, JA)), ("C", slice(NA + 40, N))):
                rows[fg][k].append(np.mean((m[sl] - th[sl]) ** 2))
    for fg in FG:
        print(f"SCALAR {fg}: " + "  ".join(f"{k} RMSE {math.sqrt(np.mean(v)):.4f}" for k, v in rows[fg].items()), flush=True)
elif RIG == "async":
    ...
    for fg in FG:
        ...
        for seed in (0, 10, 20, 30, 40):
            ...
            F, Q = rig.nominal_model(); f = LF(fg, dynamics=F, H=rig.H, process=Q, measurement=rig.SIGMA ** 2, timestep=rig.NOMINAL)
        ...
        print(f"ASYNC {fg}: whole {np.mean(whole)/0.0345:.2f}x  hot {np.mean(hot)/0.0241:.2f}x", flush=True)
elif RIG == "dyn":
    ...
    for fg in FG:
        ...
        L = LF(fg, dynamics=None, process=[[0.09]], measurement=[0.25]).filter(Y)
        W = LF(fg, dynamics=[[1.0]], process=[[0.09]], measurement=[0.25]).filter(Y)
        Y2, x2 = ar1(T=1200, a=0.6, seed=1)
        L2 = LF(fg, dynamics=None, process=[[0.09]], measurement=[0.25]).filter(Y2)
        O2 = LF(fg, dynamics=[[0.6]], process=[[0.09]], measurement=[0.25]).filter(Y2)
        print(f"DYN {fg}: test1 learned {rmse(L.mean[:,0],x):.4f} vs walk {rmse(W.mean[:,0],x):.4f} (must be <); "
    ...
    for fg in FG:
        f = LF(fg, dynamics=AR.F, control=AR.B, H=AR.measure, process=AR.Q0, measurement=AR.R0)
        r = f.filter(Y, U); est = np.asarray(r.mean)
        if not np.all(np.isfinite(est)):
            print(f"ARM {fg}: NaN", flush=True); continue
        ...
        print(f"ARM {fg}: " + "  ".join(f"{k} {m54.rms(Pl,Pt,m54.mask(sp))/m54.rms(Po,Pt,m54.mask(sp)):.3f}" for k, sp in W.items())
