...
(**Theorem A′**, [`output/03`](proofs/03-logloss-shape-minimaxity.md),
...
[`output/03`](proofs/03-logloss-shape-minimaxity.md) by the same three lines
— both equalizers exist because both losses reach the shape only through
$\mathbb E_p[e_t^2]$.)

[`output/01-shape-minimaxity.md`](proofs/01-shape-minimaxity.md). Given the
