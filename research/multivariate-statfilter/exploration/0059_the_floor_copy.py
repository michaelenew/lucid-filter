"""ATTRIBUTION GRID v4 -- the held axes walk on their OWN score at the memory's resolution floor 1/mem (the slowest
rate the bank can distinguish from zero), not at the eager sibling's smoothed scale (v3, which imported the eager
copy's excursions).  v3 docstring follows.
  Two copies per class cell (a budget).  The MEMORY copy holds
...
''' % FLOOR_SHARE)
        ...
        src = src.replace(o, '''            Kmu = self._Pmu[:, k] / (self._Pmu[:, k] + 1.0 / info)
            ...
            self.mu[:, k] += np.where(self._floor[:, k], step / self._mem, step)   # a held axis walks at the memory's resolution floor
''')
...
print("attribution grid v4 (floor axes at the memory's resolution floor) vs shipped", flush=True)
if RIG == "scalar":
