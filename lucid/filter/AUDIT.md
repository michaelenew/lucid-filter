...
| `_SPAN_S = 3.0` | proxy | ±3σ support of the class prior on the log-scale (99.7% mass); node count then follows from span/gap (budget). The support/tail-loss trade is not characterised. **Measured (resolvable-regime 0012–0016):** span is a code-length *plateau* on the scalar hero rig (\|t\| ≤ 1.46 across span 1.5–12, `0014`) and 6.0 is that rig's jump preference (`0015`) — but the plateau is **rig-local**: span 6.0 costs the asynchronous multi-rate rig 1.16× → 3.77× oracle (`0016`), because `_SPAN_S` also sets `_Pmu_cap` (∝ span²) and `_Ifloor`, loosening the walk on partial events. 3.0 holds every rig. AUD-1 stays open; the plateau does not close it. | AUD-1 |
