...
import numpy as np
sys.path.insert(0,".")
SRC="lucid/filter/lucid.py"; CB=float(sys.argv[1])
def load(variant):
    src=open(SRC).read()
    if variant=="C":
        src=src.replace("        budget = self.gap if mo == m else self.gap * (mo / m)","        budget = (%r * self.s_ax * np.sqrt(1.0 - self.phi_ax ** 2)); budget = budget if mo == m else budget * (mo / m)"%CB, 1)
        src=src.replace("        budget = self.gap if mo == m else self.gap * (mo / m)","        budget = (%r * self._s_ax * np.sqrt(1.0 - self._phi_ax ** 2)); budget = budget if mo == m else budget * (mo / m)"%CB, 1)
        pass
...
A=load("A"); C=load("C")
print(f"budget = {CB} * s*sqrt(1-phi^2) per step (class innovation sd) vs shipped 1.5 s; both engines patched:", open(SRC).read().count("budget = self.gap if mo == m else self.gap * (mo / m)"), flush=True)
# --- scalar, paired
